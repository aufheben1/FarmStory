...
public class TutorialController3 : MonoBehaviour {
	...
	IEnumerator StartNextRoutine(){
		...
		case 0 :
			SetClickListenerTile (4,4);
			tempTouch = (GameObject) Instantiate(touch, GetCoordinate (4,4) + new Vector3(0,0,-15), transform.rotation);
			Destroy(GameObject.Find ("Arrow(Clone)"));
			break;
		case 1 :
			Destroy (tempTouch);
			gameController.SendMessage ("OnTileMouseUpAsButton", new Vector2(4,4), SendMessageOptions.DontRequireReceiver);
			...
			ChangeCoverBox (stateCover [tutorialState - 1], false);
			ChangeCoverBox (stateCover [tutorialState], true);
			SetClickListenerTile (6,3);
			tempTouch = (GameObject) Instantiate(touch, GetCoordinate (6,3) + new Vector3(0,0,-15), transform.rotation);

			break;
		case 3:
			Destroy (tempTouch);
			gameController.SendMessage ("OnTileMouseUpAsButton", new Vector2(6,3), SendMessageOptions.DontRequireReceiver);
			yield return new WaitForSeconds(0.2f);
			Destroy(GameObject.Find ("Arrow(Clone)"));
			yield return new WaitForSeconds(2f);
			OnSkipButtonClick();
